...
namespace StoryPointCycleTime
{
    ...
    class Program
    {
        ...
        private static QueryResponseJson GetWorkItemListByQuery(Controller azdoController)
        {
            ...
            string wiql = "{\"query\": \"SELECT [System.Id] FROM workitems WHERE [System.WorkItemType] IN ('User Story', 'Product Backlog Item') AND [System.State] IN ('Done', 'Resolved') AND [System.AreaPath] UNDER 'EnterpriseArchitecture_2641\\\\1.SEA' AND [Microsoft.VSTS.Scheduling.StoryPoints] <> ''\"}";
        ...
        private static void CreateExcelFile(QueryResponseJson results, string excelfile, Controller azdoController, ILog logger)
        {
            try
            {
                var items = GetCycleTimeDataList(results, azdoController, logger);
                //var items = GetTestCycleTimeData();
                using (var excel = new StoryPointCycleTimeReportBuilder(excelfile))
                {
